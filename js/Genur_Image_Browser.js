/**
 * Genur.art Image Browser - ComfyUI Custom Node Frontend
...
// ----- SECTION: Constants -----
const EXT_NAME = "GenurImageBrowser.InfiniteScroll";
const DISPLAY_NAME = "🎨 Genur.art Image Browser";
const TARGET_CLASS = "GenurImageBrowser";
const COLORS = { neon: "#ff6b9d", neon2: "#c678dd" };
...
#${uid} .cg-input.cg-search{flex:1;min-width:150px}
...
#${uid} .cg-img{width:100%;height:auto;display:block;background:#0e0f13}
...
#${uid} .cg-modal-img-container img{max-width:100% !important;max-height:100% !important;object-fit:contain !important;display:block !important}
...
const CONTROLS_HTML = `
<div class="cg-row">
  <input class="cg-input cg-search" placeholder="Search..." type="text">
  <label>Sort</label><select class="cg-select cg-sort"><option value="top">Popular</option><option value="newest">Newest</option><option value="oldest">Oldest</option><option value="relevance">Relevant</option></select>
  <label>BaseModel</label><select class="cg-select cg-basemodel"><option value="">Any</option></select>
  <button class="cg-btn cg-apply">Search</button>
</div>
<div class="cg-row">
  <button class="cg-btn toggle cg-toggle-nsfw">Show NSFW</button>
...
app.registerExtension({
    ...
    beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            if (!cg.colored_once) { node.color = "#1a0a1a"; node.bgcolor = "#0d050d"; cg.colored_once = true; }

            removeSelectionPort(node);
            const wSel = ensureHiddenSelectionWidget(node, cg);

            const uid = `genur-${Math.random().toString(36).slice(2, 9)}`;
...
<div class="cg-root">
  ...
  ${MODAL_HTML}
            ...
            node.addDOMWidget("genur_images", "div", root, {});
            node.size = [1000, 750];

            const $ = (s) => root.querySelector(s);
            const elSort = $(".cg-sort"), elBaseModel = $(".cg-basemodel");
            const elSearchInput = $(".cg-search");
            const elApply = $(".cg-apply"), elRefresh = $(".cg-refresh"), elStatus = $(".cg-status");
            const elScroll = $(".cg-scroll"), elGrid = $(".cg-masonry"), elSentinel = $(".cg-sentinel");
            const elBtnNsfw = $(".cg-toggle-nsfw"), elBtnNoPrompt = $(".cg-toggle-noprompt"), elBtnFavOnly = $(".cg-toggle-favonly");
            ...
            let loading = false, hasMore = true, renderEnabled = true, currentPage = 1;
            let showNsfw = false, hideNoPrompt = false, favoritesOnly = false;
            let favoritesMap = {}, favoritesArray = [], favOffset = 0;
            ...
            const loadFavs = async () => { try { favoritesMap = await getJSON("/genur_images/get_all_favorites_data"); } catch { favoritesMap = {}; } };
            ...
            const isNsfw = (it) => it.is_nsfw === true;
            const hasPrompt = (it) => { const p = it?.prompt; return p !== null && p !== undefined && String(p).trim() !== ""; };
            const genurUrl = (it) => `https://genur.art/post/${it.id}`;

            const makeUrl = () => {
                const p = { page: currentPage, sort: elSort.value, q: elSearchInput.value.trim() };
                if (elBaseModel.value) p.baseModel = elBaseModel.value;
                return `/genur_images/images_stream?${qs(p)}`;
            };
            ...
            const selectItem = (item, cardEl) => {
                ...
                const fullUrl = item.full_url || item.url;
                setPayload({ item: { ...item, url: fullUrl, meta: { ...meta, prompt: item.prompt || meta.prompt || "", negativePrompt: meta.negativePrompt || "" } }, download_image: !!imageConnected });
                setStatus(`✓ Selected #${item.id}`);
            };

            const showModal = async (item, card) => {
                modalItem = item; modalCard = card;
                elModalPreview.src = item.url;
                elModalJson.innerHTML = formatJsonHtml(item);
                elModalOverlay.classList.add("show");
                // Fetch full details
                try {
                    const detailed = await getJSON(`/genur_images/get_post_details?id=${item.id}`);
                    if (detailed && !detailed.error) { modalItem = detailed; elModalJson.innerHTML = formatJsonHtml(detailed); }
                } catch { }
            };
            ...
            const makeCard = (it) => {
                ...
                if (isNsfw(it)) { const chip = document.createElement("span"); chip.className = "cg-chip nsfw"; chip.textContent = "NSFW"; left.appendChild(chip); }
                const baseModel = it.base_model || it.baseModel;
                if (baseModel) { const chip = document.createElement("span"); chip.className = "cg-chip"; chip.textContent = baseModel; left.appendChild(chip); }
                ...
                star.addEventListener("click", async (e) => {
                    e.stopPropagation();
                    try {
                        const resp = await api.fetchApi("/genur_images/toggle_favorite", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ item: it }) });
                ...
                const open = document.createElement("a"); open.className = "cg-open"; open.href = genurUrl(it); open.target = "_blank"; open.textContent = "Open ↗"; open.addEventListener("click", (e) => e.stopPropagation());
            ...
            const loadMore = async () => {
                ...
                try {
                    if (favoritesOnly) {
                        ...
                        if (!showNsfw) filtered = filtered.filter((i) => !isNsfw(i));
                        ...
                        if (!showNsfw) items = items.filter((i) => !isNsfw(i));
                        if (hideNoPrompt) items = items.filter(hasPrompt);
                        appendGrid(items);
                        hasMore = data?.metadata?.hasMore ?? false;
                        if (hasMore) currentPage++;
                        setStatus(hasMore ? `Loaded ${items.length} (page ${data?.metadata?.page})` : "End");
                    }
            ...
            // Event listeners
            [elSort, elBaseModel].forEach((x) => x.addEventListener("change", () => reload(true)));
            elApply.addEventListener("click", () => reload(true));
            elRefresh.addEventListener("click", () => reload(true));
            elSearchInput.addEventListener("keydown", (e) => { if (e.key === "Enter") reload(true); });
            elBtnNsfw.addEventListener("click", () => { showNsfw = !showNsfw; toggleBtn(elBtnNsfw, showNsfw); reload(true); });
            ...
            // Initialize
            toggleBtn(elBtnNsfw, showNsfw); toggleBtn(elBtnNoPrompt, hideNoPrompt); toggleBtn(elBtnFavOnly, favoritesOnly);
            loadModels();
